...
(defn- patch-env
  ...
  (def env (merge-into environ {"JANET_PATH" (cnf/dyn:modpath)
...
(defn run-patched
  ...
  (def environ (merge-into (os/environ) {"JANET_PATH" (cnf/dyn:modpath)}))
  (os/execute
    [(cnf/dyn:janet) "-e" monkey-patch ;extra-args]
    :ep
    environ))
