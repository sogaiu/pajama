...
(defn generate-config
  ...
  (def prefix-guess
    (let [de (dyn :executable)
      ...
      (cond
        (string/has-suffix? suffix-win de)
        (string/replace-all "\\" "/"
                            (string/slice de 0 (- -1 (length suffix-win))))
        (string/has-suffix? suffix-posix de)
        (string/slice de 0 (- -1 (length suffix-posix)))
        "/usr/local")))

  (def prefix
    (dyn :prefix (os/getenv "JANET_PREFIX"
                            (os/getenv "PREFIX" prefix-guess))))

  # Inherit from dyns and env variables
  (def pkglist
    (dyn :pkglist (os/getenv "JANET_PKGLIST"
                             "https://github.com/janet-lang/pkgs.git")))
  (def manpath
    (dyn :manpath (os/getenv "JANET_MANPATH"
                             (if win-prefix
                               (string win-prefix "/docs")
                               (string prefix "/share/man/man1")))))
  (def headerpath
    (dyn :headerpath (os/getenv "JANET_HEADERPATH"
                                (if win-prefix
                                  (string win-prefix "/C")
                                  (string prefix "/include/janet")))))
  (def binpath
    (dyn :binpath (os/getenv "JANET_BINPATH"
                             (if win-prefix
                               (string win-prefix "/bin")
                               (string prefix "/bin")))))
  (def libpath
    (dyn :libpath (os/getenv "JANET_LIBPATH"
                             (if win-prefix
                               (string win-prefix "/C")
                               (string prefix "/lib")))))
  (def fix-modpath
    (dyn :fix-modpath (os/getenv "JANET_STRICT_MODPATH")))
  (def modpath
    (dyn :modpath (os/getenv "JANET_MODPATH"
                             (when fix-modpath
                               (if win-prefix
  ...
  (def config
    @{:ar (if iswin "lib.exe" "ar")
      ...
      :dynamic-lflags (case hostos
                        :windows @["/DLL"]
                        :macos @["-shared"
                                 "-undefined" "dynamic_lookup""-lpthread"]
                        :mingw @["-shared"]
                        @["-shared" "-lpthread"])
      :gitpath "git"
  ...
  (if as-data
    ...
    (do
      ...
      (def fmt-str
        ``
        # Autogenerated by generate-config in pjm/make-config.janet
        (def config %.99m)
        ``)
      (def output (buffer/format buf fmt-str config))
      output)))
